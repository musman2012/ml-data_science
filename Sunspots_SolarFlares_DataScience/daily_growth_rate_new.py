...
for noaa in flares.NOAA:
    ...
    if flares.type[counter] == 'M':
        if noaa in m_flares_day:
            m_flares_day[noaa].append(flares.date[counter])
        else:
            m_flares_day[noaa] = [flares.date[counter]]
...
for counter in range(1,len(sunspots.NOAA)) :
    ...
    if sunspots.NOAA[counter] == prev_noaa:  # the same sunspot, check different date
        if sunspots.date[counter] == prev_day:
            ...
            elif sunspots.NOAA[counter] in m_flares_day: # the same sunpot is present in m flares
                temp = m_flares_day[sunspots.NOAA[counter]]
                for day in temp:
                    if day == prev_day or day == sunspots.date[counter]:
                        diff = abs(sunspots.Size[counter] - prev_size)
                        #diff = (diff / prev_size)*100
...
print("Results for x flares {} {} {} {}".format(counter1_25, counter25_50, counter51_75, counter75_100))
...
print("Results for m flares {} {} {} {}".format(counter1_25, counter25_50, counter51_75, counter75_100))
